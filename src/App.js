...
export default function App() {
  ...
  return (
    <>
      ...
      <Main movies={movies} watched={watched}></Main>
    </>
  );
}
...
function Main({ movies, watched }) {
  return (
    <main className="main">
      <SearchBox movies={movies}></SearchBox>
      <WatchBox watched={watched}></WatchBox>
    </main>
  );
}
...
function MovieList({ movies, children }) {
  return (
    <ul className="list">
      {movies?.map((movie) => (
...
function MovieStat({ watched, avgImdbRating, avgUserRating, avgRuntime }) {
  return (
    <div>
      <p>
        <span>⭐️</span>
    ...
    <div className="box">
      ...
      {isOpen1 && (
        <MovieList movies={movies}>
          {/*Children component*/}
          {/*passing movie from MovieList Map*/}
          {(movie) => <MovieYear movie={movie} />}
        </MovieList>
      )}
    </div>
  );
